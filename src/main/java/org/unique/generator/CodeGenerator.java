...
public class CodeGenerator {
    ...
    private static String dbDialect = ConfigUtil.getValue("db.dialect");
    ...
    public boolean generator(List<Table> tableList, String module, String soaPath) {
        ...
        try {
            ...
            for (int i = 0, j = tableList.size(); i < j; i++) {
                ...
                if (StringUtils.isBlank(dbDialect) || dbDialect.equals("mysql")) {
                    VelocityInfoOp.generatorCode("mysqlMapper.vm", map, soaPath + table.getPackagePath() + "/" + module + "/dao/map",
                            table.getClassNameFirstLower() + "EntityMysql.xml");
                } else if (dbDialect.equals("oracle")) {
                    VelocityInfoOp.generatorCode("oracleMapper.vm", map, soaPath + table.getPackagePath() + "/" + module + "/dao/map",
                            table.getClassNameFirstLower() + "EntityOracle.xml");
                }
